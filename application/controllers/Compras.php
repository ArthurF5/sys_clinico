...
class Compras extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('model_compras','model_empresa','model_config','model_egresos'));
		//cache 
	...
	public function lista_compras() { 
		...
		$lista = $this->model_compras->m_cargar_compras($datos, $paramPaginate);
		$fContador = $this->model_compras->m_count_compras($datos, $paramPaginate);
		$arrListado = array(); 
		foreach ($lista as $row) { 
			...
			array_push($arrListado, 
				array( 
					...
					'total_a_pagar_str' => $row['total_a_pagar_str'],
	...
	public function listar_compra_autocomplete() { 
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		$lista = $this->model_compras->m_cargar_compra_autocomplete($allInputs);
	...
	public function lista_detalle_compras()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		$datos = $allInputs['datos'];
		$paramPaginate = $allInputs['paginate'];
		$lista = $this->model_compras->m_cargar_detalle_compras($datos, $paramPaginate);
		$fContador = $this->model_compras->m_count_detalle_compras($datos, $paramPaginate);
		$arrListado = array();
		foreach ($lista as $row) { 
			$objEstado = array();
			array_push($arrListado, 
	...
	public function lista_detalle_de_una_compra()
	{
		$allInputs = json_decode(trim($this->input->raw_input_stream),true);
		$datos = $allInputs['datos'];
		$notaCredito = $allInputs['sensor']; //para saber si es para una nota de credito
		$lista = $this->model_compras->m_cargar_detalle_de_una_compra($datos, $notaCredito);
		$arrListado = array();
		foreach ($lista as $row) { 
			$objEstado = array();
			array_push($arrListado, 
	...
	public function ver_popup_detalle_compra()
	{
		$this->load->view('compra/detalleCompra_formView');
	}
	public function ver_popup_nueva_compra()
	{
		$this->load->view('compra/compra_formView');
	}
	public function registrar_compra()
	{
    	...
    	// VALIDAR QUE LA NOTA DE CREDITO/DEBITO SEA MAYOR A CERO 
    	...
    	// El Total de la Nota de Credito no debe ser mayor al de Documento origen
    	// NO SIRVE ESTA VALIDACION ESTA MAL PLANTEADA 
    	// if($allInputs['tipodocumento']['id'] == 7 || $allInputs['tipodocumento']['id'] == 14){
    	// 	/*calculo total en notas de credito anteriores*/
    	// 	$notasCredito = $this->model_compras->m_calcular_total_notas_credito($allInputs['itemCompra']['idmovimiento']);
    	// 	$notasDebito = $this->model_compras->m_calcular_total_notas_debito($allInputs['itemCompra']['idmovimiento']);
    		
    	// 	if($allInputs['total'] > ((float)$allInputs['itemCompra']['total_a_pagar'] + (float)$notasDebito['suma_total']  + (float)$notasCredito['suma_total'])) {
    	// 		$arrData['message'] = 'No puede ingresar una nota de crédito/débito con un monto mayor al total. Corrija los montos e intente nuevamente.';
	    // 		$arrData['flag'] = 0;
	    // 		$this->output
				 //    ->set_content_type('application/json')
				 //    ->set_output(json_encode($arrData));
		...
		if( $this->model_compras->m_registrar_compra($allInputs) ){ 
			...
			foreach ($allInputs['detalle'] as $key => $row) { 
				...
				if( $this->model_compras->m_registrar_detalle_compra($row) ){ 
	...
	public function cambiar_estado_compra()
	{
    	...
    	if( $allInputs['num_estado'] == '2' ){ // A OBSERVADO 
    		if( $this->model_compras->m_cambiar_estado_compra_a_observado($allInputs) ){ 
				$arrData['message'] = 'Se cambió de estado correctamente'; 
	    		$arrData['flag'] = 1;
			}
    	}elseif( $allInputs['num_estado'] == '3' ){ // A APROBADO 
    		if( $this->model_compras->m_cambiar_estado_compra_a_aprobado($allInputs) ){ 
				$arrData['message'] = 'Se cambió de estado correctamente'; 
	    		$arrData['flag'] = 1;
			}
    	}elseif( $allInputs['num_estado'] == '4' ){ // A PAGADO 
    		if( $this->model_compras->m_cambiar_estado_compra_a_pagado($allInputs) ){ 
				$arrData['message'] = 'Se cambió de estado correctamente'; 
	    		$arrData['flag'] = 1;
			}
    	}elseif( $allInputs['num_estado'] == '0' ){ // A ANULADO 
    		if( $this->model_compras->m_anular_compra($allInputs['idmovimiento']) ){ 
				$arrData['message'] = 'Se cambió de estado correctamente'; 
	    		$arrData['flag'] = 1;
			}
    	} 
	...
	public function exportar_excel(){
	    ...
	    $objWriter->save('assets/img/dinamic/excelTemporales/listadoCompras_'.$dateTime.'.xls'); 
	    $arrData = array(
	      'urlTempEXCEL'=> 'assets/img/dinamic/excelTemporales/listadoCompras_'.$dateTime.'.xls',
	      'flag'=> 1
	    );    	
